...
Read in your data file using `read_csv("classdata_2018.csv")`, and assign it to the dataframe name `IntroSurveyRaw` using the `<-` left-arrow operator.
...
Run the code below to create a new variable for overall Regret Score per the Regret Scale in the `IntroSurvey` dataframe called `regret_total`.
...
```{r}
IntroSurvey <- mutate(IntroSurveyRaw,
