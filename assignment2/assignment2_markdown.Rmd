...
## 4. Calculating overall scores for the Regret and Maximization scales
...
To do this, we'll use the `ifelse()` function with `mutate()` to generate a new column in our data, like we did earlier. The code below tells R to create a new variable (`school_binary`) whose value is `"B_CC"` _if_ the original variable (`school`) was either Barnard or CC, and whose value is `"GS_SPS"` _otherwise._
...
```{r}
IntroSurvey <- mutate(IntroSurvey,
                      dec_mode_2 = ifelse(startsWith(dec_mode, "calculation-based"),
                                          "calc",
                                          "not-calc"))
```
