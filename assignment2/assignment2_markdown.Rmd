...
Read in your data file using `read_csv("classdata_2018.csv")`, and assign it to the dataframe name `IntroSurvey` using the `<-` left-arrow operator.
...
Run the code below to create two new columns in `IntroSurvey`:

* `regret_total`, for overall Regret Score on the Regret Scale (`regret1` to `regret5`)
* `maxi_total`, for overall Maximizing score on the Maximizing Scale
You'll take the approach used above to calculate an overall Maximizing score from the six items we took from the short version of the Maximizing Scale. They are called `maxi1`, `maxi2`, `maxi3`, etc.

First, assign a new variable called that is the average of all six `maxi` scores (if you need help, see the code above for calculating the mean regret score).
...
You will be using the function `mutate()` to **add new columns to a dataframe,** or **overwrite existing columns in a dataframe.** Anytime you want to calculate a new column that may be some combination of existing columns, like averaging the values across multiple columns, `mutate()` is the function you want.
